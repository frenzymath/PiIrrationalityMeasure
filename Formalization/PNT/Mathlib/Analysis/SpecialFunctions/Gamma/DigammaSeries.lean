...
`digamma` with the limit of the derivatives there. Only the right half-plane statement
is retained in this adaptation.

## Main statement

* `Complex.hasSum_digamma_of_re_pos`: the series representation for `0 < z.re`.
-/
